...
Filter_fold="results/01_Drug_Targets"
...
for (database_name in database_list) {
  ...
  database_file <- paste0("药物靶点预测/", database_name, ".csv")
...
write.csv(combined_df,"results/01_Drug_Targets/02_Database_Targets_Summary.csv",row.names = FALSE)  #保留文件
...
unique_target <-combined_df%>% 
                ...
                distinct(across(Gene_symbol, ~toupper(.)), .keep_all = TRUE)


write.csv(unique_target,"results/01_Drug_Targets/01_Unique_Drug_Targets.csv",row.names = FALSE)  #保留文件
#-------------------------------------------------------------------------------
...
ggsave("results/01_Drug_Targets/03_Database_Target_Stats.png", width = 8, height = 8, dpi = 1000, bg = "white")
ggsave("results/01_Drug_Targets/03_Database_Target_Stats.pdf", width = 8, height = 8, dpi = 1000, bg = "white")
#-------------------------------------------------------------------------------
...
#---------------------------成分-靶点数量统计-----------------------------------
ingredient_target <-combined_df%>% 
                    dplyr::select(Mol_ID, Gene_symbol)%>% 
                    distinct()


count_df <- ingredient_target  %>%
  count(Mol_ID, name = "Count")



ggplot(count_df, aes(x = Mol_ID, y = Count)) +
  geom_col(width = 0.5, fill = "#4682b4") + # 柱子填充颜色
  geom_text(aes(label = Count), # 添加文本图层
            vjust = -0.5, # 调整文本位置，使其位于柱子顶部
            color = "black",size = 2) + # 设置文本颜色，确保在深色背景上可见
  ...
  theme_minimal() +
  theme(panel.border = element_rect(colour = "black", linewidth = 0.5, fill = NA), # 添加外框线
        axis.text = element_text(hjust = 1, angle = 45, size = 6, colour = "black"),
        axis.line = element_line(colour = "black", linewidth = 0.5),
        axis.ticks = element_line(colour = "black", linewidth = 0.5),
        axis.ticks.length = unit(0.2, "cm"),
        axis.title = element_text(color = "black", size = 14),  #设置坐标轴标题格式
...
ggsave("results/01_Drug_Targets/04_Ingredient_Target_Stats.png", width = 20, height = 8, dpi = 1000, bg = "white")
ggsave("results/01_Drug_Targets/04_Ingredient_Target_Stats.pdf", width = 20, height = 8, dpi = 1000, bg = "white")
#-------------------------------------------------------------------------------
