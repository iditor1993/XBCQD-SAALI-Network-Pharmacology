setwd("C:/Users/Administrator/Desktop/网络药理学")                                #设置工作目录
...
#----------------------------------桑基图---------------------------------------
#用于桑基图的数据表
sankey_df <- output_data_RNA%>%
  dplyr::select(Gene_symbol, KEGGID)%>%
  dplyr::distinct()


df1 <- sankey_df %>% dplyr::select(1,2) %>% group_by(Gene_symbol,KEGGID) %>% count() %>%
  pivot_stages_longer(.,stages_from = c("Gene_symbol", "KEGGID"),
                      values_from = "n")


# 确定需要的颜色数量
num_colors_needed <- length(unique(c(df1$node)))

colors <- c("#FFFF66", "#FFCCFF", "#FFCC99", "#FFCC33", "#FF99FF", "#FF9999", "#FF9933", "#FF66FF", "#FF6666", "#FF33CC", "#FF3333", "#CCFFFF", "#CCFF99", "#CCFF33", "#CCCCFF", "#CCCC99", "#CCCC33", "#CC99FF", "#CC9999", "#CC9933", "#CC66FF", "#CC6699", "#99FFFF", "#99FF99", "#99FF33", "#99CCFF", "#99CC99", "#9999FF", "#999999", "#999966", "#999900", "#9966CC", "#996633", "#9933CC", "#9900FF", "#99FFFF", "#99FFCC", "#99FF66", "#99CC33", "#999999", "#9966CC", "#996633", "#00FF99", "#00CCCC", "#00CC66", "#009999", "#0066CC", "#006666", "#0033FF", "#003399", "#FFFF66", "#FFCCFF", "#FFCC99", "#FFCC33", "#FF99FF", "#FF9999", "#FF9933", "#FF66FF", "#FF6666", "#FF33CC", "#FF3333", "#CCFFFF", "#CCFF99", "#CCFF33", "#CCCCFF", "#CCCC99")
...
p1 <- ggplot(data=df1,aes(x = stage,y =n,group = node,
  ...
  geom_text(data=df1 %>% filter(connector=="to"),
            ...
            position = position_sankey(v_space ="auto",order="ascending", nudge_x=0),
            hjust=0.5,size=3.5,vjust=0.5,color="black",fontface="bold")+
...
node_data2 <- g$data[[which(sapply(g$data, function(x) "y" %in% names(x)))[4]]]
node_data <- node_data2 %>% dplyr::select(label,y)

#将刻度值添加到富集分析表格中
enrich_df <- merge(kegg_top20,node_data,by.x = "KEGGID",by.y = "label")
...
p2<-plot_df %>%
  ...
  geom_point(data = plot_df,
             aes(x = GeneRatio, y = y, fill = neg_log10_pvalue, size = Count), shape = 21, colour = "white") + 
  ...
  scale_size(range = c(2, 5), guide = guide_legend(override.aes = list(colour = "black"))) +
  theme_bw() + 
  theme(
