...
#保存kegg富集气泡图的路径
KEGGdot_png <- "results/03_Intersection_Targets/02_GO_KEGG_Enrichment/02_KEGG/KEGG_dot.png"
KEGGdot_pdf <- "results/03_Intersection_Targets/02_GO_KEGG_Enrichment/02_KEGG/KEGG_dot.pdf"
#-------------------------------------------------------------------------------
...
#---------------------------读取文件、整理数据----------------------------------
  ...
  plot_df <- kegg_top20 %>%
    dplyr::arrange(desc(neg_log10_pvalue)) %>%              #每一个Category都按照GeneRatio进行降序排序
...
#-----------------------------------绘图----------------------------------------
  ...
  p<-plot_df %>%
    ...
    geom_point(data = plot_df,
               aes(x = GeneRatio, y = Description, fill = neg_log10_pvalue, size = Count), shape = 21, colour="white") + 
    scale_fill_gradient(low = "yellow", high = "red", name = "-log10(pvalue)",labels = function(x) sprintf("%.1f", x),breaks = breaks) + 
    ggtitle(label = "  ") + 
    labs(x = "GeneRatio", y = "Description") + 
    scale_size(range = c(2, 7),guide = guide_legend(override.aes = list(colour = "#000000",fill= "#000000"))) +  #设置count图例的格式
    theme_bw() + 
  ...
  ggsave(KEGGdot_png,width = 8, height = 8, dpi = 300,bg = "white")      #在这里调整图片宽度、高度
  ggsave(KEGGdot_pdf,width = 8, height = 8, dpi = 300,bg = "white")
#-------------------------------------------------------------------------------
