...
#------------------------创建文件夹、定义输入\输出文件路径----------------------
Filter_fold="results/04_Core_Targets/03_Core_PPI"
...
Edge_Node_count_path <- "results/04_Core_Targets/03_Core_PPI/Edge_Node_count.csv"
#-------------------------------------------------------------------------------
...
#--------------------------------读取输入文件-----------------------------------
Final_core_target <- read.csv(Final_core_target_path, stringsAsFactors = FALSE)
...
#--------------------------------进行PPI分析------------------------------------
#将string_protein_添加到Final_core_target
Final_core_target <- merge(Final_core_target,
...
#写入文件
write.table(count_df, Edge_Node_count_path, row.names = FALSE, sep = "\t")
