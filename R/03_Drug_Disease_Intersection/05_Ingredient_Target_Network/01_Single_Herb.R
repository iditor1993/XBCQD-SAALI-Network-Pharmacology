...
#保存为文件
write.xlsx(ingredient_target_net,network_path,rowNames = FALSE)
...
#生成typek文件
#ingredient的type
ingredient_type <- list()
ingredient_type$node <- ingredient_target_net$from_node
ingredient_type$type <- "ingredient"
ingredient_type <- as.data.frame(ingredient_type)%>%
  distinct()


#target的type
target_type <- list()
target_type$node <- ingredient_target_net$to_node
