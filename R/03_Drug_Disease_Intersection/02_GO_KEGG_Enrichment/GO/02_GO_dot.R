...
#保存GO富集气泡图的路径
godot_png <- "results/03_Intersection_Targets/02_GO_KEGG_Enrichment/01_GO/GO_dot.png"
godot_pdf <- "results/03_Intersection_Targets/02_GO_KEGG_Enrichment/01_GO/GO_dot.pdf"
#-------------------------------------------------------------------------------
...
#---------------------------读取文件、整理数据----------------------------------
  ...
  go_enrich$neg_log10_pvalue <- -log10(go_enrich$pvalue)
  ...
  plot_df <- GO_top10 %>%
    ...
    dplyr::mutate(Description = str_remove(Description, pattern = "\\(.*")) %>%     #删除Description的（以及以后得内容
...
#-----------------------------------绘图----------------------------------------
  #以下是提取出每个ONTOLOGY的neg_log10_pvalue的最大值和最小值，用于图例的范围设置
  BP_MAX <- plot_df %>%
    filter(ONTOLOGY == "BP") %>%          # 筛选出ONTOLOGY为"BP"的行
    pull(neg_log10_pvalue) %>%            # 提取Pvalue列
    max()                                 # 计算最大值
  ...
  BP_breaks <- seq(BP_MIN,BP_MAX ,length.out = 4) # 这将生成一个包含5个元素的向量，用于4个分割点
  CC_breaks <- seq(CC_MIN,CC_MAX ,length.out = 4) # 这将生成一个包含5个元素的向量，用于4个分割点
  MF_breaks <- seq(MF_MIN,MF_MAX ,length.out = 4) # 这将生成一个包含5个元素的向量，用于4个分割点
  ...
  p<-plot_df %>%
    ...
    geom_point(data = plot_df %>% dplyr::filter( ONTOLOGY == "MF"),
               aes(x = GeneRatio, y = interaction(Description,  ONTOLOGY), fill = neg_log10_pvalue, size = Count), shape = 21,colour="#228351") + 
    scale_fill_gradient(low = "#b2eecf", high = "#41ae76", name = "MF -log10(pvalue)",labels = function(x) sprintf("%.1f", x),breaks = MF_breaks) +       #添加一个颜色渐变图例 
    ggnewscale::new_scale_fill() +       #新增一个图层
    geom_point(data = plot_df %>% dplyr::filter( ONTOLOGY == "CC"),
               aes(x = GeneRatio,, y = interaction(Description,  ONTOLOGY), fill = neg_log10_pvalue, size = Count), shape = 21,colour="#076d79") + 
    scale_fill_gradient(low = "#90dee7", high = "#0a8a99", name = "CC -log10(pvalue)",labels = function(x) sprintf("%.1f", x),breaks = CC_breaks) + 
    ggnewscale::new_scale_fill() +
    geom_point(data = plot_df %>% dplyr::filter( ONTOLOGY == "BP"),
               aes(x = GeneRatio, y = interaction(Description,  ONTOLOGY), fill = neg_log10_pvalue, size = Count), shape = 21, colour="#cf3403") + 
    scale_fill_gradient(low = "#f2c7b9", high = "#e74716", name = "BP -log10(pvalue)",labels = function(x) sprintf("%.1f", x),breaks = BP_breaks) + 
    guides(y = "axis_nested") + 
    ggtitle(label = "  ") + 
    labs(x = "GeneRatio", y = "Description") + 
    scale_size(range = c(2, 6),guide = guide_legend(override.aes = list(colour = "#000000",fill= "#000000"))) +  #设置count图例的格式
    theme_bw() + 
  ...
  ggsave(godot_png,width = 10, height = 9, dpi = 300,bg = "white")      #在这里调整图片宽度、高度
  ggsave(godot_pdf,width = 10, height = 9, dpi = 300,bg = "white")
#-------------------------------------------------------------------------------  
